...
namespace AdminApp.Infrastructure.Repositories
{
    public class UserRepository : _BaseRepository<User>, IUserRepository
    {
        ...
        {
            return await _context.User.Include(i => i.Role).ToListAsync();
        }

        public override async Task<User> GetByAsync(Expression<Func<User, bool>> func)
        {
            return await _context.User.Include(i => i.Role).FirstOrDefaultAsync(func);
        }

        public override async Task<User> GetByIdAsync(int id)
        {
            return await _context.User.Include(i => i.Role).FirstOrDefaultAsync(i => i.UserId == id);
        }
